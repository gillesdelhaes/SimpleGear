...
async def create_location(body: LocationCreate, session: AsyncSession = Depends(get_session), _=Depends(require_admin)):
...
async def update_location(location_id: int, body: LocationUpdate, session: AsyncSession = Depends(get_session), _=Depends(require_admin)):
...
async def delete_location(location_id: int, session: AsyncSession = Depends(get_session), _=Depends(require_admin)):
    result = await session.execute(select(Location).where(Location.id == location_id))
    loc = result.scalar_one_or_none()
    if not loc:
        raise HTTPException(status_code=404, detail="Location not found")

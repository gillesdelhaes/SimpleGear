...
async def create_status(body: StatusCreate, session: AsyncSession = Depends(get_session), _=Depends(require_admin)):
...
async def update_status(status_id: int, body: StatusUpdate, session: AsyncSession = Depends(get_session), _=Depends(require_admin)):
...
async def delete_status(status_id: int, session: AsyncSession = Depends(get_session), _=Depends(require_admin)):
    result = await session.execute(select(AssetStatus).where(AssetStatus.id == status_id))
    s = result.scalar_one_or_none()
    if not s:
        raise HTTPException(status_code=404, detail="Status not found")

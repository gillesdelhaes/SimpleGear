...
import type { AssetStatus, AssetCategory, AssetModel, AuthUser } from '../types'
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-neutral-500">{models.length} model{models.length !== 1 ? 's' : ''} defined</p>
        <button onClick={() => setShowCreate(true)} className="px-4 py-2 rounded-xl gradient-bg text-white text-sm font-semibold hover:opacity-90 flex items-center gap-1.5">
          <svg width="13" height="13" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" /></svg>
          New model
        </button>
      ...
      {models.length === 0 ? (
        ...
        <div className="space-y-4">
          {Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([manufacturer, items]) => (
            <div key={manufacturer} className="bg-white rounded-2xl border border-neutral-100">
...
type Tab = 'models' | 'statuses' | 'categories' | 'users'
